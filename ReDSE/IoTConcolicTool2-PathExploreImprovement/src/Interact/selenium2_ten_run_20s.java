...
public class selenium2_ten_run_20s {
    ...
    private String current_sim = "";
    private boolean isTesting = false;
    private int currentTestNum = 0;
    private int time;
    private List<Map<String, WebElement>> sim_maps = new ArrayList<>();
    private Map<String, WebElement> selectMap = new HashMap<>();
    private Map<String, WebElement> play_map = new HashMap<>();

    public selenium2_ten_run_20s(int time, ArrayList<String> preferencesInputs, ArrayList<String> simulatorsInputs, String codePath, String defaultValue) {
    ...
    public void test() {
        ...
        try{
            ...
            WebElement pre_sign = driver.findElement(By.xpath("/html/body/div[1]/main/div/div/div[1]/form/fieldset/div[6]"));
            ...
            isTesting = true;
            //run t
           // int j = 0;// no log count;
            for(int i = 0; i < time; i++){
                WebElement el = ((ChromeDriver) driver).findElementById("ide-log-header");
                WebElement clear = el.findElement(By.tagName("button"));
                clear.click();
                Thread.sleep(500);
                String log_tmp = change_simulator_state(driver);
                if (log_tmp!=null&&!log_tmp.equals("")){
                    log += log_tmp;
                }
                clear.click();
                Thread.sleep(500);
    ...
    private void clearApps(WebDriver driver) throws Exception{
        ...
        while(hasEdit(sheet)){
            ...
            WebElement bar = null;
            try {
                (new WebDriverWait(driver, 10)).until(ExpectedConditions.visibilityOfElementLocated(By.className("form-actions")));
                Thread.sleep(100);
            ...
            sheet = null;
            try {
                (new WebDriverWait(driver,10)).until(ExpectedConditions.visibilityOfElementLocated(By.id("smartapp-table")));
                sheet = driver.findElement(By.id("smartapp-table"));
            }catch (Exception e){}
        }
    }
...
//        System.out.println("nothing...");
    ...
    private void simulate(WebDriver driver) throws Exception {
        ...
        WebDriverWait wait_preference = new WebDriverWait(driver, 20);
    ...
    private void set_simulators_get_log(WebDriver driver) throws Exception {
        ...
        if(simulator_settings.size() == 0){
            if(exploredSimInput.size() == 0){
                current_sim = set_simulator_no_sim_input(0, driver, selectMap, play_map, log_map);
            }else{
                select_by_index(currentTestNum, driver, selectMap, play_map, log_map);
            }
...
//        uninstall.click();
    ...
    private String set_simulator_no_sim_input(int time, WebDriver driver, Map<String, WebElement> selectMap, Map<String, WebElement> playMap, Map<String, String> log_map) throws Exception{
        String current_sim = "";
        if(time == 3) return current_sim;
        ...
        for(Map.Entry<String, WebElement> entry: selectMap.entrySet()){
            ...
            if(log_info.length() != 0){
                log_map.put(current_sim, log_info);
                return current_sim;
            }
        }

        return set_simulator_no_sim_input(time + 1, driver, selectMap, playMap, log_map);
    ...
    private void select_by_index(int run, WebDriver driver, Map<String, WebElement> selectMap, Map<String, WebElement> playMap, Map<String, String> log_map) throws Exception{
        int i = 0;
        for(Map.Entry<String, WebElement> entry: selectMap.entrySet()){
            String key = entry.getKey();
            WebElement select = entry.getValue();
            ...
            WebElement play = playMap.get(key);
            Select mselect = new Select(select);
            mselect.selectByIndex(exploredSimInput.get(i));
            play.click();
            Thread.sleep(2000);
            i++;
        }
        WebElement console_log = driver.findElement(By.xpath("/html/body/div[5]/div/div/div[2]/div/div[2]/div[2]/div"));
        String log_info = console_log.getText();
        log_map.put(current_sim + run, log_info);
    }

    private String change_simulator_state(WebDriver driver) throws Exception{
        int i = 0;
        for(String sim: selectMap.keySet()){
            WebElement select = selectMap.get(sim);
            WebElement play = play_map.get(sim);
            Select mselect = new Select(select);
            if(i>=exploredSimInput.size()) break;
            int index = exploredSimInput.get(i) + 1;
            if(index >= mselect.getOptions().size())
                index = 0;
            mselect.selectByIndex(index);
            play.click();
            Thread.sleep(15000);
            WebElement console_log = driver.findElement(By.xpath("/html/body/div[5]/div/div/div[2]/div/div[2]/div[2]/div"));
            String log_info = console_log.getText();
            if(log_info!=null&&!log_info.equals("")) {
                return log_info;
            }
            i++;
        }
